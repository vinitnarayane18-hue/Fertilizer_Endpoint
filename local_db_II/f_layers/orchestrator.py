...
# Ensure the package root (local_db_II) is on sys.path so imports work
PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

import lookup
from f_layers import scraper, gemini_advisor
...
class FertilizerAdvisor:
    ...
    def get_recommendation(self, crop, disease=None, use_scraper=True, use_gemini=True):
        # ---------- Layer 1: local DB ----------
